...
function resolveGlobalClaudeCliPath(): string | undefined {
  try {
    ...
    // Use spawnSync to avoid shell injection when resolving symlinks
    const readlinkResult = spawnSync("readlink", ["-f", claudeBin], { encoding: "utf8" });
    const realpathResult = spawnSync("realpath", [claudeBin], { encoding: "utf8" });
    const realBin = (readlinkResult.status === 0 ? readlinkResult.stdout.trim() : null)
      ?? (realpathResult.status === 0 ? realpathResult.stdout.trim() : null)
      ?? claudeBin;
